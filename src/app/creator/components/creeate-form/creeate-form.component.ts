...
export class CreeateFormComponent implements OnInit {
  ...
  numberErrors = {
    MSISDN_LINEA: 'El numero de telefono de la sim ya esta tomado',
    NUMERO_DE_DOCUMENTO: 'El numero de documento ya existe',
    variable: '',
  };
  ...
  constructor(
    ...
    public numeroschips: ChipsService,
  ...
  ngOnInit() {
    ...
    this.formGroup.controls.NOMBRE_COMPLETO.valueChanges.subscribe(
      (value: string) => {
    ...
    this.formGroup.controls.DIRECCION_BARRIO_BENEFICIARIO.valueChanges.subscribe(
      (value: string) => {
        this.formGroup.controls.DIRECCION_BARRIO_BENEFICIARIO.setValue(
          value.toUpperCase(),
          { emitEvent: false }
        );
      }
    ...
    this.formGroup.controls.NOMBRE_COMPLETO_TUTOR.valueChanges.subscribe(
      (value: string) => {
        this.formGroup.controls.NOMBRE_COMPLETO_TUTOR.setValue(
          value.toUpperCase(),
          { emitEvent: false }
        );
      }
  ...
  onKey(event) {
    ...
    if (!event.key.match(pattern)) {
      const data: string = this.formGroup.get('NOMBRE_COMPLETO').value;
  ...
  nombreTutor(event) {
    const pattern = '[A-Za-z ]';
    if (!event.key.match(pattern)) {
      const data: string = this.formGroup.get('NOMBRE_COMPLETO_TUTOR').value;
  ...
  create() {
    ...
    data.DEPARTAMENTO_BENEFICIARIO = this.departamento.getDepartamento(
      this.formGroup.get('MUNICIPIO_BENEFICIARIO').value
    );
    data.SERIAL_SIM = this.numeroschips.getSerial(
      this.formGroup.get('MSISDN_LINEA').value
    );
    this.creatorservice.createfile(data).subscribe((res) => {
      if (res['success'] === true) {
        ...
        this.numberErrors.variable = res['message'];
      }
    });
  }
}
